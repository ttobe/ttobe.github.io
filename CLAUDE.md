...
`01 경력`(넛지헬스케어, 2025.05~, 파트장/개발자 2역할 + 업무) → `02 주요 업무`(CASE 01 Nginx 499 / 02 GA4 DAU / 03 어뷰징 클러스터링 Union-Find / 04 업무 요청 방식 개선) → `03 프로젝트`(fancamp 1-1~1-4 / kopilot 2-1~2-2 / ME:RROR 3-1) → `04 글·아티클`(velog 글 카드, `.pf-writing`) → `05 경험·학력`(부스트캠프·KT Aivle·생명정보학 연구생 / 홍익대).
